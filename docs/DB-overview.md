...
| 関係 | 意味 |
|------|------|
| プレイヤー → キャラクター | 1人が複数キャラを所持（`characters.player_id`） |
...
| **player_profiles** | アイコン・自己紹介・志向・ティア・お気に入り |
...
| 画面・用途 | 主な参照先 |
|------------|------------|
| ホーム / 投稿 | `posts`, `recent_comments_with_names` |
| プレイヤー詳細 | `player_detail_summary` + 必要に応じ個別テーブル |
