...
- 卓の参加者の**正**は `run_players` / `run_characters`（Worker が洗替）。`runs` の配列列は非推奨（DROP 候補）
...
| **runs** | 卓。タイトル・GM・状態を保持（参加者は junction） |
| **run_players** | 卓×プレイヤーの順序付きjunction（書込みの正） |
| **run_characters** | 卓×キャラクターの順序付きjunction（書込みの正） |
...
| 配列列 | **DROP 済**（2026-07-21）。API 応答キーは junction から組み立て |
| 順序 | `sort_order`（同一IDは先頭のみ） |
| 所有者列 | junction の `user_id` は親runの `user_id` をコピー |

詳細は [`junction-read-progress.md`](./junction-read-progress.md)。
...
| 名前 | 種別 | 内容 |
|------|------|------|
| **sync_run_arrays_to_junctions** | FUNCTION | （旧）配列→junction 同期。**DROP 済**（2026-07-21） |
| **trg_runs_sync_arrays_to_junctions** | TRIGGER | （旧）配列変更後の洗替。**DROP 済**（2026-07-21） |

復元が必要なら `database-optimization.md` A-3。手順メモ: [`sql/disable-array-to-junction-trigger-2026-07.sql.md`](./sql/disable-array-to-junction-trigger-2026-07.sql.md)。
...
| 互換期間 | junction を正としつつ配列列をミラー／フォールバックとして残している期間 |
