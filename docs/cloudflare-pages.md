...
コード・CI・公開URLの正本はすでに Cloudflare 側（フロント `fctzs` / API `fctzs-trpg`）です。  
締めは **人手確認と旧 GitHub Pages の停止**だけです。
...
完了したら [`platform-roadmap.md`](./platform-roadmap.md) のフロント公開行を「移行完了」に更新してよい。
