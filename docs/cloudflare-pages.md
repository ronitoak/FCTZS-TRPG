...
静的フロントだけを `dist/` に集め、Cloudflare Pages へ公開する。

| 役割 | URL / 名前 |
|------|------------|
| フロント（Pages） | プロジェクト名 `fctzs` → [https://fctzs.daruji.workers.dev/](https://fctzs.daruji.workers.dev/) |
| API（Worker） | [https://fctzs-trpg.daruji.workers.dev](https://fctzs-trpg.daruji.workers.dev)（従来どおり） |
...
| 結果 | 意味 | 次にやること |
|------|------|----------------|
| 全体が緑（成功） | `dist` 生成 → Pages `fctzs` へ公開完了 | [https://fctzs.daruji.workers.dev/](https://fctzs.daruji.workers.dev/) をハードリロードして確認 |
| `Authentication error` / `401` / `403` | Token または Account ID が違う | Secrets を作り直し、Token 権限に Pages Edit があるか確認 |
| `Project not found` | プロジェクト名不一致 | Dashboard の Pages 名が正確に `fctzs` か確認 |
...
短期間だけ並行公開し、問題なければ GitHub Pages を停止する。
