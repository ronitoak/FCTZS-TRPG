...
| タブ | 内容 |
|------|------|
| ホーム | ゲスト: 直近・進行中・コメント。ログイン＋連携時: **自分の次回予定・募集・スタイルおすすめ・予定カレンダー（一括入力）**＋部の予定。AppBar からスケジュール照合・Discordログイン |
| PL | プレイヤー一覧 / 詳細（プロフィール・キャラ・参加卓・**コメント**） |
| シナリオ | 一覧検索 / 詳細（紹介・関連卓・通過キャラ・**気になるトグル**・**コメント**） |
| セッション | 進行中／終了済の卓 / 卓詳細（GM・PL・キャラ・開催URL・**コメント**） |
| 募集 | 一覧検索 / 詳細（**応募／取消**・応募者・**コメント**） |
| キャラ | 一覧検索 / 詳細（能力・技能・通過シナリオ・**コメント**） |
...
- ~~Discord ログイン + コメント投稿~~ → 詳細5画面で実装済み
- ~~気になる / 募集応募~~ → 実装済み
- ~~ホーム個人ダッシュボード~~ → 次回予定・自分の募集・スタイルおすすめ
- ~~予定カレンダー／一括入力~~ → ホーム埋め込み（昼/夜）
- プロフィール編集、またはキャラ／シナリオ作成
