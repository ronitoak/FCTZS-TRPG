...
(function () {
  ...
  async function fetchPlayerAvailabilities(playerId, year = null, month = null) {
    if (!playerId) return [];
    let query = `select=*&player_id=eq.${encodeURIComponent(playerId)}`;
  ...
  async function getCurrentUserPlayerContext({ players = null, loadProfile = true } = {}) {
    ...
    const profiles = await apiGet("player_profiles");
  ...
  async function apiFetchJson(path, options = {}) {
    ...
    // init-supabase.jsが保持する同一セッションを使い、画面間で認証状態がずれないようにする。
    if (!window.supabase) {
