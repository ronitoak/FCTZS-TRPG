...
(function () {
   ...
   * ヘッダーの描画（認証状態を反映）
   */
   function functionrenderHeader(targetId, relativePath, session) {
    const nav = document.getElementById(targetId);
    if (!nav) return;
    ...
    // ログイン・ユーザー情報の出し分け[cite: 12]
    if (session) {
      const user = session.user.user_metadata;
      const avatar = user.avatar_url || "";
      const name = user.full_name || user.name || "User";
      
      html += ` | <span class="user-nav-info">
        <img src="${avatar}" class="nav-avatar" title="${this.escapeHtml(name)}">
        <button id="logout-btn" class="btn-small">Logout</button>
      </span>`;
    } else {
      html += ` | <button id="login-btn" class="btn-small">Discord Login</button>`;
    }
